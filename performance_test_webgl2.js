...
var netCPU = TestObject(1, 5, [3, 4000, 4000, 1]);
var netWebGL2 = TestObject(1, 5, [3, 4000, 4000, 1]);
...
for (let r = 0; r < netCPU.runs; r++) {
  ...
  for (let p = 0; p < netCPU.passes; p++) {
    inputData.forEach((item, i) => {
      net.trainCPU(item, [givenPoints[i]]);
    });
  }
  ...
  predictFor.forEach(async (item, i) => {
    netCPU.results.push(Math.round(net.forwardPropagationCPU(item)[netCPU.structure.length - 1][0]));
  });
