...
var netCPU = TestObject(1, 5, [3, 2048, 2048, 1]);
var netWebGL2 = TestObject(1, 5, [3, 2048, 2048, 1]);
...
let doTest = () => {
  for (let r = 0; r < netCPU.runs; r++) {
    ...
    for (let p = 0; p < netCPU.passes; p++) {
      for (let i = 0; i < inputData.length; i++) net.trainCPU(inputData[i], [givenPoints[i]]);
    }
  
    let t1 = performance.now();
    netCPU.results = [];

    for (let i = 0; i < predictFor.length; i++) netCPU.results.push(Math.round((net.predictCPU(predictFor[i]))[0]));
  ...
  for (let r = 0; r < netWebGL2.runs; r++) {
    ...
    for (let p = 0; p < netWebGL2.passes; p++) {
      for (let i = 0; i < inputData.length; i++) net.trainGPU(inputData[i], [givenPoints[i]]);
    }
    // net.saveTraining();
    ...
    netWebGL2.results = [];
    for (let i = 0; i < predictFor.length; i++) netWebGL2.results.push(Math.round((net.predictGPU(predictFor[i]))[0]));
